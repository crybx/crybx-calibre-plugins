...
def parse_args(args):
    ...
    from calibre_plugins.epubaholic.dialogs import ALL_OPTIONS
...
def copy_cover(cover_path):
    ...
    # standard Epubaholic logic will delete file at location passed.
...
def invoke_modify_epub(epub_path, options, cover_path, quiet):
    from calibre_plugins.epubaholic.modify import modify_epub
    if quiet:
        log = Log(Log.ERROR)
    else:
        log = Log()
...
def main():
    ...
    try:
        ...
        # If a path to a cover specified, make a temporary copy as Epubaholic will delete it
        temp_cover_path = None
        if cover_path:
            temp_cover_path = copy_cover(cover_path)

        # Invoke the Epubaholic plugin
