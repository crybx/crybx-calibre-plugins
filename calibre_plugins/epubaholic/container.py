...
class Container(object):
    ...
    source code. However, having overridden so many methods to fix bugs or alter
...
class WritableContainer(Container):
    ...
    def add_to_manifest(self, id, href, mt=None):
        '''
        Given an id, href and optional media-type, create an item in the manifest for it
        '''
        manifest = self.opf.xpath('//opf:manifest', namespaces={'opf':OPF_NS})[0]
        item = manifest.makeelement('{%s}item'%OPF_NS, nsmap={'opf':OPF_NS},
                href=href, id=id)
    ...
    def add_to_toc(self, title, href):
        '''
        Given an id, title and href, add something like this to the TOC
        <navPoint id="num_71" playOrder="71">
          <navLabel>
            <text>#069</text>
          </navLabel>
          <content src="text/auto_69.html"/>
        </navPoint>
        '''
        if self.ncx_name is None:
            self.log('\t  No TOC found')
            return
        ncx = self.ncx
        navmap = ncx.xpath('//ncx:navMap', namespaces={'ncx':NCX_NS})[0]
        navpointcount = len(navmap.xpath('ncx:navPoint', namespaces={'ncx':NCX_NS}))
        navpoint = ncx.makeelement('{%s}navPoint'%NCX_NS, nsmap={'ncx':NCX_NS})
        navmap.append(navpoint)

        navlabel = ncx.makeelement('{%s}navLabel'%NCX_NS, nsmap={'ncx':NCX_NS})
        navpoint.append(navlabel)

        text = ncx.makeelement('{%s}text'%NCX_NS, nsmap={'ncx':NCX_NS})
        text.text = title
        navlabel.append(text)

        content = ncx.makeelement('{%s}content'%NCX_NS, nsmap={'ncx':NCX_NS},
                src=self.name_to_href(href, rel_to_opf=False))
        navpoint.append(content)

        # Add the navpoint attributes
        # id should be 'num_' + navpointcount + 1
        # playOrder should be the playOrder of the previous navpoint + 1
        point_id = 'num_%d'%(navpointcount + 1)
        play_order = navpointcount + 1
        navpoint.set('id', point_id)
        navpoint.set('playOrder', str(play_order))
        self.log('\t  TOC Navpoint inserted: %s:%s:%s'%(point_id, title, href))
        self.set(self.ncx_name, self.ncx)

    def add_to_nav(self, title, href):
        '''
        Add a nav item to the spine and TOC
        <body><nav><ol>
        <li><a href="chapter_69.html">#069</a></li>
        </ol></nav></body>
        '''
        for html_name in self.get_html_names():
            if 'nav.xhtml' in html_name:
                html = self.get_raw(html_name)
                href_rel = href[href.rfind('/') + 1:]
                add = '<li><a href="%s">%s</a></li>\n\t</ol>' % (href_rel, title)
                html = html.replace('</ol>', add)
                self.set(html_name, html)
                self.log('\t  Nav item inserted: %s:%s' % (href_rel, title))
...
class ExtendedContainer(WritableContainer):
    '''
    Extend the container object with additional functions that
    assist with working with sets of content specific to Epubaholic
    '''
