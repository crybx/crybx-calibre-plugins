...
class DownloadProgressDialog(QDialog):
    ...
    def __init__(self, parent, books_data, config, db):
        ...
        self.setWindowTitle('Downloading MangaDex Metadata')
        ...
        self.worker = FetchWorker(books_data, parent=self)
...
class BookDetailDialog(QDialog):
    ...
    def __init__(self, parent, book_id, book_title, data, config, db, can_apply=False):
        ...
        from calibre_plugins.mangadex.common_lang import resolve_lang_for_column as _resolve_lang_for_column
        from calibre.utils.localization import calibre_langcode_to_name
        lang_code        = md_lang_to_calibre(data.get('original_language') or '')
...
class ApplyMetadataDialog(QDialog):
    ...
    def __init__(self, parent, results, books_data, config, db, apply_book_fn=None):
        ...
        self.setWindowTitle('Apply MangaDex Metadata')
...
class _SearchWorker(QThread):
    ...
    def run(self):
        from calibre_plugins.mangadex.scraper import search_md_series
        try:
            results = search_md_series(self.query)
...
class SearchLinkDialog(QDialog):
    '''
    Search MangaDex and link a series to each selected book.
    ...
    def __init__(self, parent, books_data, config, db):
        ...
        self.setWindowTitle('Link to MangaDex')
        ...
        self._table.setHorizontalHeaderLabels(['Title', 'Genre', 'Year', 'Status'])
    ...
    def _do_search(self):
        ...
        self._worker = _SearchWorker(query, parent=self)
    ...
    def _write_link(self, book_id, mu_url):
        ...
        try:
            db.set_identifier(book_id, 'mangadex', mu_url, index_is_id=True)
        except Exception:
            pass

        from calibre_plugins.mangadex.config import KEY_LINK_COL
        link_col = self._config.get(KEY_LINK_COL, '#links').strip()
        if not link_col:
            return

        md_link = '[MangaDex](%s)' % mu_url
        try:
            ...
            if existing:
                ...
                mu_link_re = re.compile(
                    r'\[MangaDex\]\([^)]*mangadex\.org[^)]*\)', re.IGNORECASE)
            ...
            print('MangaDex: failed to write link to %s: %s' % (link_col, e))
...
class _MetadataFetchWorker(QThread):
    ...
    def run(self):
        ...
        try:
            data = fetch_md_metadata(self.manga_id)
            if data:
                data['_url'] = 'https://mangadex.org/title/%s' % self.manga_id
                data['_manga_id'] = self.manga_id
...
# ---------------------------------------------------------------------------
# Add from MangaDex dialog
# ---------------------------------------------------------------------------

class SeriesPreviewDialog(QDialog):
    '''
    Preview metadata from a MangaDex series page with per-field
    ...
    def __init__(self, parent, data, config):
        ...
        if data.get('alt_titles'):
            lbl = QLabel('<i>Also known as: ' + ', '.join(data['alt_titles']) + '</i>')
            lbl.setWordWrap(True)
...
class AddFromMDDialog(QDialog):
    '''
    Search MangaDex, preview a series, and create a new empty book
    ...
    def __init__(self, parent, config, db, apply_fn=None):
        ...
        self.setWindowTitle('Add from MangaDex')
        self.setMinimumSize(750, 500)
    ...
    def _do_search(self):
        ...
        self._search_worker = _SearchWorker(query, parent=self)
    ...
    def _on_double_click(self, row, _col):
        ...
        self._fetch_worker = _MetadataFetchWorker(result['manga_id'], parent=self)
    ...
    def _create_book(self, data):
        ...
        if self._apply_fn:
            from calibre_plugins.mangadex.config import DEFAULT_STORE_VALUES
