...
from qt.core import (
    QAction, QDialog, QDialogButtonBox, QInputDialog, QLabel,
...
class ConfigDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.setWindowTitle('FixFilenames - Configure Rules')
        self.resize(700, 500)
...
class RenameByChapterTool(Tool):
    ...
    def run(self):
        ...
        if renamed == 0 and failed == 0:
            info_dialog(self.gui, 'Rename by chapter',
                'No matching files found to rename.', show=True)
            return
...
class SortSpineTool(Tool):
    ...
    def create_action(self, for_toolbar=True):
        ac = QAction('Sort spine by filename', self.gui)
        if not for_toolbar:
    ...
    def run(self):
        selected = self.gui.file_list.file_list.selected_names
        if not selected:
            info_dialog(self.gui, 'Sort spine',
                'No files selected. Select files in the File browser first.', show=True)
            return
        self.boss.commit_all_editors_to_container()
        try:
    ...
    def sort_spine(self, selected):
        self.boss.add_savepoint('Before: Sort spine by filename')
        container = self.current_container
        ...
        if all(a is b for a, b in zip(selected_elements, sorted_elements)):
            info_dialog(self.gui, 'Sort spine', 'Spine is already sorted.', show=True)
            return None
...
class ConfigureRenameTool(Tool):
    ...
    def create_action(self, for_toolbar=True):
        ac = QAction('Configure rename rules...', self.gui)
        ac.triggered.connect(self.run)
        return ac
